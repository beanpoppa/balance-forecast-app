...
export default function App() {
  const [startingBalance, setStartingBalance] = useState(() => load("bf_startingBalance", 3000));
  const [lowBalanceThreshold, setLowBalanceThreshold] = useState(() => load("bf_lowBalanceThreshold", 500));
  const [forecastDays, setForecastDays] = useState(() => load("bf_forecastDays", 60));
  const [items, setItems] = useState(() => load("bf_items", defaultItems));
  const [nextId, setNextId] = useState(() => load("bf_nextId", 10));

  useEffect(() => { localStorage.setItem("bf_startingBalance", JSON.stringify(startingBalance)); }, [startingBalance]);
  useEffect(() => { localStorage.setItem("bf_lowBalanceThreshold", JSON.stringify(lowBalanceThreshold)); }, [lowBalanceThreshold]);
  useEffect(() => { localStorage.setItem("bf_forecastDays", JSON.stringify(forecastDays)); }, [forecastDays]);
  useEffect(() => { localStorage.setItem("bf_items", JSON.stringify(items)); }, [items]);
  useEffect(() => { localStorage.setItem("bf_nextId", JSON.stringify(nextId)); }, [nextId]);
  ...
  return (
<div style={{ minHeight: "100vh", width: "100%", boxSizing: "border-box", background: "#0a0f1a", color: "#e2e8f0", fontFamily: "'DM Sans', sans-serif", padding: "32px 20px" }}>
